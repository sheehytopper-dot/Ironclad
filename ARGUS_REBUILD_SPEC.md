...
**Phase 2 — Market machinery:** MLPs, rollover blending, absorption, general vacancy/credit loss with offsets, full recovery structures, % rent. GATE: goldens #2, #4, #5 match in full (cash flow, Lease Audit, Recovery Audit — §9.1); the percentage-rent module is built with the manual's worked-example unit tests (Iron Rule 3) but is **externally unvalidated pending golden #3** (§9.1 standing intake).
