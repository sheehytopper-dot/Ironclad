...
**Validation reports**
24. Benchmark Comparison — loads an expected cash flow file (e.g. a transcribed OM cash flow, `expected_annual_cash_flow.csv`) alongside a property JSON, runs the engine, and produces a line-by-line diff with per-line tolerance flags (default: the §9.1 golden tolerance, $500 per line at annual fiscal-year level; tolerance configurable). Purpose: every future deal validates against its source OM's published Argus output before assumptions are toggled (§9.1 fixture-lock rule).
...
1. **Four OM-based goldens spanning complexity** (reduced from five 2026-07-09 — golden #5, Inland Logistics, permanently disqualified: no Argus attribution anywhere in its OM; DEVIATIONS.md §14), each an Offering Memorandum with a published Argus-based cash flow: (#1) single-tenant NNN (`tests/golden/clorox_northlake/`); (#2) multi-tenant with base-year or expense-stop recoveries; (#3) retail with percentage rent — **standing opportunistic intake, no deadline** (owner decision 2026-07-05: the first qualifying retail OM the owner obtains is staged, transcribed under the fixture-lock rule, and back-tested via the Benchmark Comparison report, §7 report 24); (#4) chosen from deal triage for coverage of gross-ups, caps, or absorption.
...
4. **Fixture-lock rule (standing policy): transcribed inputs are human-verified against the source pages and committed before any engine comparison runs.** Every future deal validates against its source OM's published Argus output via the Benchmark Comparison report (§7 report 24) before assumptions are toggled.
5. **No UI work begins until golden test #1 (simple property) passes end-to-end. No report work beyond Cash Flow until golden test #2 passes.**
...
**Phase 2 — Market machinery:** MLPs, rollover blending, absorption, general vacancy/credit loss with offsets, full recovery structures, % rent. GATE: goldens #2, #4 cash flows match the OM within tolerance (§9.1; reduced from three goldens 2026-07-09 — #5 disqualified, DEVIATIONS.md §14); Lease Audit and Recovery Audit reports built, reconciling exactly to the ledger, and owner-reviewed; the percentage-rent module is built with the manual's worked-example unit tests (Iron Rule 3) but is **externally unvalidated pending golden #3** (§9.1 standing intake).
