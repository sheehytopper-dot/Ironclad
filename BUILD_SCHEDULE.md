...
>
> **Current status (2026-07-12): GATE 3 PASSED — owner declaration 2026-07-12** after review of the Gate 3 evidence (NEXT_STEPS_TO_GATE3.md Step 7). All six Phase 3 build steps shipped 2026-07-11/07-12: TI/LC posting (golden #1 capital lines green within $0.50/cell); purchase / closing costs / security deposits; the debt engine (fixed/floating, IO, balloon, additional principal, loan costs; "Other Debt" deliberately excluded); property resale (all five methods + the Property Resale Audit report reconciling to 1e-9); PV / IRR / direct cap (all six discount conventions, nominal IRR annualization); and sensitivity matrices (value + unleveraged/leveraged IRR grids, cross-checked cell-by-cell against single-point valuation). The §9.3 invariant set is extended and standing on every run (debt balance roll, payoff-at-resale, PV/IRR 1bp self-consistency). Full suite 392 passed / 4 failed — the four by-design golden reds: Freeport Gate 2 (137, deferred-B), Cedar Alt Gate 2 (47, deferred-B), Freeport Gate 3 capital (33, root cause E, deferred to beta-stage GUI testing), Cedar Alt Gate 3 capital (12, root cause D, closed as C's sibling); none is a blocker. The three owner hand-checks (amort, resale, PV/IRR) are confirmed. **Price-derivation scope decision (owner decision 2026-07-12, DEVIATIONS.md §20 #6): `Purchase.derivation != fixed` and `LoanAmountBasis.pct_of_value` refuse permanently — no current deal backs out price from valuation or sizes a loan off it, and building it now is real architecture with zero current pull; a permanent boundary, not an open gap.** **Phase 4 begins** (spec §10: full §7 report catalog + Excel export) — Iron Rule 2: no Phase 4 code until NEXT_STEPS_TO_PHASE4.md is drafted and reviewed.
>
> **Current status (2026-07-16): GATE 4 PASSED — owner declaration 2026-07-16** after the §8 workbook spot-check (the substitute comparison source confirmed at Phase 4 Step 0 — owner spot-check + exact reconciliation + the golden-CSV anchor; no ARGUS prints exist). Phase 4 complete across seven build steps plus corrections: the report-builder contract + unit/period toggle engine; the §8 priority report set (18 of 24 builders — the Step-0-deferred six #10/#13/#14/#17/#19/#22 excluded by owner decision); Cash Flow + Benchmark Comparison anchored to the golden CSVs (reproducing the by-design red counts exactly); the values-only Excel export package with cell-by-cell read-back tests; both intake surfaces (PropertyModel JSON + the §5.2 rent-roll template round-trip with readable, non-programmer errors); the report/export/intake polish pass (Contractual/Speculative provenance from the RunResult, DEVIATIONS §25 named tradeoff); and the DEVIATIONS §25 standing test-discipline rules born from this phase's two review findings. Suite 573 passed / 4 failed — the same four by-design golden reds (Freeport Gate 2 137, Cedar Alt Gate 2 47, Freeport Gate 3 capital 33, Cedar Alt Gate 3 capital 12), none a blocker. **Phase 5 begins (Streamlit UI, spec §6) — Iron Rule 2: planning only until NEXT_STEPS_TO_PHASE5.md is owner-reviewed; no UI code before that.**
>
...
**Superseded 2026-07-11, then CLOSED 2026-07-12 — GATE 3 PASSED (see the Current-status paragraph above and [NEXT_STEPS_TO_GATE3.md](NEXT_STEPS_TO_GATE3.md) Step 7):** none of the three golden OMs publishes any valuation result, so the "Your jobs" reconciliation line and the GATE 3 checklist below (both predating this finding, preserved here as historical record) were read through NEXT_STEPS_TO_GATE3.md's actual criteria instead — golden capital-line tolerance, §9.3 invariants, manual worked examples, and owner hand-checks, not an OM price/IRR/matrix match — and Gate 3 was declared passed on those criteria 2026-07-12. The stale checklist below is retained unchanged (not deleted) per the project's transparency-in-editing rule; the actual satisfied criteria are in NEXT_STEPS_TO_GATE3.md Step 7.
...
**GATE 4 (end of week 9): PASSED — owner declaration 2026-07-16.**
- [x] All 23 reports render and export — **as scoped at Phase 4 Step 0 (owner decision 2026-07-13): the priority set, 18 of 24 builders; #10/#13/#14/#17/#19/#22 deferred to a later pass** (#17 because percentage rent itself is externally unvalidated pending golden #3). Every built report reconciles to its RunResult/ledger source; Cash Flow + Benchmark Comparison anchor to the golden CSVs
- [x] Result package workbook opens clean, formatted, correct units in headers — §8 formatting standard, values-only, unit/period noted per tab; owner spot-check on the regenerated workbooks (`scripts/build_gate4_workbook.py`) 2026-07-16
- [x] Rent roll template imports with row-level validation errors on bad data — §5.2 round-trip both directions; readable errors incl. blank-required-cell coverage (c8e0ec3) and the Contractual/Speculative provenance filter with reported-not-silent skips (DEVIATIONS §25)
